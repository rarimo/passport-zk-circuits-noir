...
function processPassport(filePath) {
    ...
    const dg1_bytes = reHex.test(json.dg1) ? Hex.decode(json.dg1) : Base64.unarmor(json.dg1);
    ...
    // get dg15 info
    const [aa_pk, aa_shift] = extractFromDg15(json.dg15);
    ...
    const compile_params = {
        ...
        ec_shift: ec_shift,
        aa_sig_type: aa_pk ? aa_pk.n ? 1 : 20 : 0,
        aa_shift: aa_shift
    }

    const old_naming_convention = `registerIdentity_${compile_params.sig_type}_${dg_hash_type * 8}_${dg1_bytes.length == 93 ? 3 : 1}_${hash_type <= 32 ? Math.ceil(ec_bytes.length / 64) : Math.ceil(ec_bytes.length / 128)}_${dg1_shift * 8}_${ec_shift * 8}_${dg15_bytes.length == 0 ? "NA" : (aa_pk.n ? 1 : 20) + "_" + dg15_shift * 8 + "_" + (dg_hash_type <= 32 ? Math.ceil(dg15_bytes.length / 64) : Math.ceil(dg15_bytes.length / 128)) + "_" + aa_shift * 8}`
