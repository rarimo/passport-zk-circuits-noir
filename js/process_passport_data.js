...
function getChunkedParams(pk, sig) {
  const ec_field_size = pk.param
    ? reHex.test(pk.param)
      ...
      : "UNKNOWN FIELD SIZE"
    : 0;
  const chunk_number = pk.x
    ? Math.ceil(pk.x.length / 30)
    : Math.ceil(pk.n.length / 30);

  const pk_chunked = pk.x
    ? bigintToArray(120, chunk_number, BigInt("0x" + pk.x)).concat(
        bigintToArray(120, chunk_number, BigInt("0x" + pk.y))
      )
    : bigintToArray(120, chunk_number, BigInt("0x" + pk.n));
  const sig_chunked = pk.x
    ? bigintToArray(120, chunk_number, BigInt("0x" + sig.r)).concat(
        bigintToArray(120, chunk_number, BigInt("0x" + sig.s))
      )
    : bigintToArray(120, chunk_number, BigInt("0x" + sig.n));
  const reduction = pk.x
    ? bigintToArray(120, chunk_number * 2, BigInt(0))
    : bigintToArray(
        120,
        chunk_number,
        compute_barret_reduction(pk.n.length * 4 + 2, BigInt("0x" + pk.n))
      );
...
function parseCSVWithNewlines(content) {
  const rows = [];
  let currentRow = [];
  let currentField = "";
  let inQuotes = false;
...
function processPassport(filePath, value) {
  ...
  const dg15_shift = dg15_bytes.length
    ? getDg15Shift(asn1_decoded, dg15_bytes, dg_hash_type)
    : 0;
  ...
  const inputs = {
    ...
    pk: JSON.stringify(
      chunked.pk_chunked.map((x) => "0x" + x.toString(16))
    ).replaceAll('"', ""),
    reduction: JSON.stringify(
      chunked.reduction.map((x) => "0x" + x.toString(16))
    ).replaceAll('"', ""),
    sig: JSON.stringify(
      chunked.sig_chunked.map((x) => "0x" + x.toString(16))
    ).replaceAll('"', ""),
